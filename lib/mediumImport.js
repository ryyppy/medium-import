...
function mediumImport(arg, onErr) {
  ...
  client.getUser((err, user) => {
    ...
    console.log('Trying to upload post... Please wait...');
    ...
    client.createPost({
      ...
      contentFormat,
      content,
      publishStatus: (publishStatus === 'DRAFT') ? medium.PostPublishStatus.DRAFT : medium.PostPublishStatus.PUBLIC,
      publishedAt
    }, (err, post) => {
        console.log(post);
      if(err){
      ...
      console.log('---');
      console.log(`It's done, ${user.username}! See your stories on https://medium.com/me/stories`);
    });
  });
}
